...
for new_file_ind = 1:numel( new_spiketimes.all_spiketimes )
  ...
  new_file_data = get_adjusted_new_units_data(new_spikedata);
...
function new_data = get_adjusted_new_units_data(new_spikedata)
new_data = struct();
new_file_data = {};
spike_ct = [];
...
for unit = 1:numel(new_spikedata)
    ...
    new_file_data{unit} = new_spikedata{idx(1)} / 400000;
end
